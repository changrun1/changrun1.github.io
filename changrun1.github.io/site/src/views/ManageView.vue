...
<template>
  <section class="space-y-10">
    ...
    <div class="rounded-3xl border border-slate-200/70 bg-white/85 p-6 shadow-sm">
      ...
      <div class="space-y-2 max-h-[420px] overflow-y-auto pr-1" v-if="filteredDownloads.length">
        ...
        >
          ...
          <RouterLink
            :to="{ path: '/preview', query: { path: file.path } }"
            class="rounded-full border border-slate-200 bg-white px-2 py-1 text-[11px] font-semibold text-slate-600 transition hover:border-indigo-300 hover:text-indigo-700"
            @click.stop
          >預覽</RouterLink>
          <button
            type="button"
            class="rounded-full border border-slate-200 bg-white px-2 py-1 text-[11px] font-semibold text-indigo-600 transition hover:border-indigo-300 hover:text-indigo-700"
    ...
    <!-- 預覽區改為提示使用外部預覽頁 -->
    <div class="rounded-3xl border border-slate-200/70 bg-white/85 p-6 shadow-sm">
      <h2 class="mb-2 text-sm font-semibold text-slate-600">預覽</h2>
      <p class="text-xs text-slate-500">點選列表「預覽」按鈕會在同 SPA 切換到 /preview 頁面顯示。</p>
      <div class="mt-4 rounded-xl border border-dashed border-slate-200 bg-white/60 p-6 text-center text-xs text-slate-400">此區不再內嵌顯示內容。</div>
    </div>
